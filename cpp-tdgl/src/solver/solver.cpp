...
void TdglSolver::solve() {
    ...
    // Clear running state buffers at start
    rs_mu_buffer_.clear();
    rs_dt_buffer_.clear();
    ...
    for (step = 0; step < max_steps && time_ < options_.solve_time; ++step) {
        ...
        // Update boundary conditions with dynamic terminal current
        if (use_timing_) {
            double j_scale = 1.0;
            if (device_.K0 > 0) {
                double current_si = unit_to_si(options_.current_units);
            ...
            // Override terminal_currents_ with timing-scheduled value
            terminal_currents_["source"] = terminal_current_at(time_);
            update_mu_boundary(j_scale);
        }
        ...
        if (options_.save_every > 0 && save_counter >= options_.save_every) {
            ...
            if (solution_writer_) {
                solution_writer_->save_step(step, time_, dt_,
                    psi_, mu_, supercurrent_, normal_current_,
                    applied_A_, A_induced_, epsilon_);
                // Flush running state buffer for this frame
                if (!rs_mu_buffer_.empty() && !rs_dt_buffer_.empty()) {
                    solution_writer_->save_running_state(
                        solution_writer_->frame_count() - 1,
                        rs_mu_buffer_, rs_dt_buffer_);
                    rs_mu_buffer_.clear();
                    rs_dt_buffer_.clear();
    ...
    if (solution_writer_) {
        solution_writer_->save_step(step, time_, dt_,
            psi_, mu_, supercurrent_, normal_current_,
            applied_A_, A_induced_, epsilon_);
        // Flush remaining running state buffer
        if (!rs_mu_buffer_.empty() && !rs_dt_buffer_.empty()) {
            solution_writer_->save_running_state(
                solution_writer_->frame_count() - 1,
                rs_mu_buffer_, rs_dt_buffer_);
