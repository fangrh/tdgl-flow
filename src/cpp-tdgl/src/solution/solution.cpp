...
SolutionWriter::SolutionWriter(const std::string& output_path, const Mesh& mesh)
    : output_path_(output_path), save_count_(0) {
...
void SolutionWriter::save_step(int step, double time, double dt,
    ...
    // Write psi as two real datasets (real and imaginary parts)
    std::vector<double> psi_real(n), psi_imag(n);
    for (int i = 0; i < n; ++i) {
        psi_real[i] = psi(i).real();
        psi_imag[i] = psi(i).imag();
    }
    write_1d<double>(grp, "psi_real", psi_real.data(), n);
    write_1d<double>(grp, "psi_imag", psi_imag.data(), n);
