...
def write_cpp_mesh(
    ...
    with h5py.File(output_path, "w") as f:
        ...
        em_group.create_dataset("directions", data=np.asarray(em.directions, dtype=np.float64))
...
def write_cpp_mesh_from_data(mesh_data: dict, output_path: str):
    ...
    with h5py.File(output_path, "w") as f:
        ...
        if "directions" in em_dict:
            em_group.create_dataset("directions", data=np.asarray(em_dict["directions"], dtype=np.float64))
